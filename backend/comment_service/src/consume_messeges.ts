...
import { addCommentFromBroker } from "./routes.js";
...
export const consumeMessages = async () => {
  try {
    ...
    const exchange = 'comment_exchange';
    ...
    await channel.consume(queue, async (msg) => {
        try {
           ...
           // Check if the required fields are present
            if (!commentData.username || !commentData.eventId) {
              channel.ack(msg);
              return;
            }
            await addCommentFromBroker(commentData);
            channel.ack(msg);
          } catch (error) {
